...
namespace LoxInterpreter {
    ...
    public:
        ...
        virtual Literal* clone() { return new Literal(d_data); }
    ...
    public:
        ...
        Token(const Token& token): d_literal(token.d_literal->clone()) { }

        ~Token() { d_literal.reset(); }

        const Token& operator=(const Token& rhs)
        {
            std::unique_ptr<Literal_> tmp(rhs.d_literal->clone());
            d_literal.reset();
            d_literal = std::move(tmp);
            return rhs;
        }
