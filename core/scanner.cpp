...
inline auto
Scanner::endOfFile() const
    -> bool
{
    return d_current >= std::end(d_source);
}
    ...
    -> void
{
    ...
    d_tokens.emplace_back(Token_Type::END_OF_FILE, "", NULL, 0);
    return;
}
...
auto
Scanner::addToken(const Token_Type type)
    -> void
{
    addToken(type, NULL);
}
...
template <typename T>
auto
Scanner::addToken(const Token_Type type, const T& literal)
    -> void
{
    using namespace std::literals::string_literals;
    auto text = {d_start, d_current - d_start}s;
    // std::string text = d_source.substr(d_start, d_current - d_start);
    std::cout << "addToken | " << text << " | " << literal << '\n';
    d_tokens.emplace_back(type, text, literal, d_line);
    return;
}
...
Scanner::matchChar(const char expected)
    -> bool
{
    if (endOfFile() || d_source[d_current] != expected)
        return false;
    d_current++;
    return true;
}
